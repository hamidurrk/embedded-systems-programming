	...
	switch (new_state) {
		...
		case FAULT:
			...
			*requested_floor = *current_floor;
			...
			for (int8_t blink_amount = 0; blink_amount < (FAULT_BLINK_DURATION_MS/FAULT_BLINK_PERIOD_MS)
			{
...
int main(void)
{
    ...
    while (1)
    {
	    ...
	    switch(elevator_state) {
		    case IDLE:
		    next_state = idle_state_transition_check(requested_floor, current_floor)break;
		    case GOINGUP:
		    case GOINGDOWN:
