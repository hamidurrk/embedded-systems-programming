...
static volatile uint8_t s_keypad_wake_flag = 0u;
...
static uint8_t keypad_DecodeScanCode(uint8_t key_scan_code);
/**************************************************************************************************/
...
uint8_t KEYPAD_GetKey()
{
	...
	var_keyPress_u8 = keypad_DecodeScanCode(var_keyPress_u8);
	return(var_keyPress_u8);                      // Return the key
}

uint8_t KEYPAD_GetKeyIfPressed()
{
	uint8_t key = keypad_DecodeScanCode(keypad_ScanKey());
	if (key == 'z')
	{
		return '\0';
	}
...
uint8_t KEYPAD_GetKeyNonBlocking()
{
	/* Return each key once per press-release cycle with minimal debounce filtering. */
	static uint8_t s_last_sample = '\0';
	static uint8_t s_stable_count = 0u;
	static uint8_t s_latched = 0u;
	uint8_t sample = KEYPAD_GetKeyIfPressed();
...
void KEYPAD_EnableWakeInterrupt(void)
{
	/* Keep rows driven low and columns with pull-up so any key press toggles PK0..PK3. */
	M_ROW = 0x0F;
	PCMSK2 |= 0x0Fu;
	PCIFR |= (1u << PCIF2);
	PCICR |= (1u << PCIE2);
...
void KEYPAD_DisableWakeInterrupt(void)
{
	PCICR &= (uint8_t)(~(1u << PCIE2));
}

void KEYPAD_ClearWakeFlag(void)
{
	s_keypad_wake_flag = 0u;
	PCIFR |= (1u << PCIF2);
}

uint8_t KEYPAD_GetWakeFlag(void)
{
	return s_keypad_wake_flag;
}

ISR(PCINT2_vect)
{
	s_keypad_wake_flag = 1u;
}
